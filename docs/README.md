...
평균 물가 상승률은 1.57%를 기준으로 계산한다. (2011년~2021년 사이의 평균 소비자 물가 상승률)
...
은퇴 이후 100세까지 필요한 돈은 은퇴하는 해의 물가 상승률에 맞춰서 계산한다. 은퇴 이후 물가상승률은 가지고 있는 자산이 경제와 맞춰서 함께 증가한다는 가정으로 따로 계산하지 않는다.
...
최소,적정 노후 생활비 : (은퇴할 나이 - 현재 나이)년 뒤 최소,적정 노후비
...
- [ ] 계산이 끝나면 로딩 스피너가 종료되며 결과를 보여줘야 한다.
- [ ] 결과를 서울,광역시,도를 구분지어 사용자에게 보여줘야 한다.
- [ ] 결과를 다른 사람들에게 공유할 수 있는 버튼이 있어야 한다.
...
- [x] 사용자가 현재 나이, 은퇴하고자 하는 나이, 개인 혹은 부부를 입력하였다면 정보를 저장하는 기능이 있어야 한다.
- [ ] 평균 물가 상승률을 고려한 N 년뒤 최소,적정 노후 생활비를 계산하는 기능이 있어야 한다.
- [ ] 서울,광역시,도에 해당하는 정보를 전부 계산해야 한다.
- [ ] 개인 혹은 부부인지에 따라 다르게 계산해야 한다.
